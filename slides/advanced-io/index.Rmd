...
* Python packages to accomplish this: `html5lib`, `BeautifulSoup`, `lxml`, `scrapy` (Python 2 only)
* Web scraping built into `pandas` with the `read_html()` function (for tables)
    * First: install `html5lib` and `beautiful-soup` from `conda`
    * Then: "Expect to do some cleanup after you call this function."
...
```python
url = 'http://www.espnfc.us/barclays-premier-league/23/table'
