...
<img src="img/04-modules-and-eda/data.jpg" style="width: 850px"><figcaption>Source: [bigdatapix.tumblr.com](http://bigdatapix.tumblr.com/)</figcaption>

---

### The data analysis process

<img src="img/04-modules-and-eda/r4ds-data-science.png" style="width: 800px"><figcaption>Source: Wickham and Grolemund, [_R for Data Science_](http://r4ds.had.co.nz/introduction.html)</figcaption>
...
<img src=img/04-modules-and-eda/pydata.PNG style="width: 700px"><figcaption>Source: [Jake VanderPlas, SciPy 2015 Keynote](https://speakerdeck.com/jakevdp/the-state-of-the-stack-scipy-2015-keynote)</figcaption>
...
::: {.incremental}
* Download `grad_rates.csv` from the link to your computer
* In Colab, click the __folder icon__ in the left sidebar to open the Files pane, then the __upload__ button, and choose the file
* It lands in `/content`, Colab's working directory - so `pd.read_csv("grad_rates.csv")` finds it
* Uploaded files __disappear when the runtime disconnects__.  Re-upload, or mount your Google Drive
:::
...
<img src=img/04-modules-and-eda/df1.png>
...
## Reading the AI's first lines

<!-- SCAFFOLDING (accurate; Kyle to re-voice, delete this comment):
     Code an agent writes for you starts with a block of imports. Each one is a package it decided
     to use on your behalf: pandas, numpy, seaborn, matplotlib, and often others you haven't met.
     help(pd.read_csv) prints the docstring, the same tool description from last week's deck.
     Takeaway: the import block is the first thing to read in AI-written code, and help() is how
     you find out what any of it does. -->

::: {.incremental}
- [KYLE: the first thing AI-written code does is `import` - that block is the list of tools it chose for you]
- [KYLE: you don't have to know every package; you do have to be able to look one up]
- [KYLE: `help(pd.read_csv)` - last week's docstring, on a function you didn't write]
:::
...
* __Interval__: additive; the zero is arbitrary (0°F is not "no temperature")
...
* Offers a standardized way to discuss the spread of a distribution.  For example, in a normally distributed sample: 
    * About 68 percent of the values will be within one standard deviation of the mean
    * About 95 percent of the values will be within two standard deviations of the mean
    * About 99.7 percent of the values will be within three standard deviations of the mean
...
## `.describe()` is your first check

<!-- SCAFFOLDING (accurate; Kyle to re-voice, delete this comment):
     This is the first of the course's verification habits: look at count / min / max / spread
     BEFORE trusting any number computed from the column. Two things this table tells you:
     (1) rate runs from 0.022 to exactly 1.000 - are those real institutions or data problems?
     (2) mean rate = 0.477. That is NOT "the graduation rate": every college counts once, whatever
     its size. A 200-student college and a 50,000-student university weigh the same. And there is
     no enrollment column here, so you *cannot* weight it - the data can't answer that question.
     An AI asked "what's the average graduation rate?" will report 0.477 without saying any of this.
     Ties to levels of measurement: rate is a ratio-level number, but it is a rate, not a count. -->

::: {.incremental}
- [KYLE: read this table before you trust anything else - count, min, max, spread]
- [KYLE: min is 0.022 and max is exactly 1.000. Real, or a data problem?]
- [KYLE: mean of `rate` is 0.477. Is that "the graduation rate"? Every college counts once, whatever its size]
- [KYLE: there's no enrollment column - so this dataset *can't* answer that question. AI will answer it anyway]
:::
...
### Before you plot: what shape do you expect?

<!-- SCAFFOLDING (Kyle to re-voice, delete this comment):
     Predict-then-run in chart form. Ask the room to sketch the shape of graduation rates across
     2,372 colleges before showing the histogram: symmetric? piled up at one end? two humps?
     Then reveal. The actual shape: roughly symmetric and wide, peak near 0.4, a slight right
     tail, and only a handful of institutions at 1.0.
     This sets up Assignment 3's "do your data appear skewed?" question as a prediction they
     already practiced. -->

- [KYLE: 2,372 colleges. Before I show you: sketch the shape you expect for graduation rates]
...
grad.rate.hist()
```
<img src=img/04-modules-and-eda/hist1.png style="width:600px">
...
<img src=img/04-modules-and-eda/hist2.png style="width:600px">
...
```python
sns.kdeplot(data = grad, x = "rate", fill = True)
```
<img src=img/04-modules-and-eda/kde.png>
...
```
<img src=img/04-modules-and-eda/boxplot.png>

---

### Anatomy of a box plot

<img src=img/04-modules-and-eda/anatomy.png>
...
<img src=img/04-modules-and-eda/violin.png>
