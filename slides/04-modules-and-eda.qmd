...
* Download link: <https://github.com/walkerke/geog30323/blob/main/data/colleges.csv> (click __Download raw file__)
...
::: {.incremental}
* Download `colleges.csv` from the link to your computer
...
* It lands in `/content`, Colab's working directory - so `pd.read_csv("colleges.csv")` finds it
...
colleges.head()
                                  ...
                                  name    rate     rnk  median_earn
0             Alabama A & M University  0.2403  2031.0      40628.0
1  University of Alabama at Birmingham  0.6423   672.0      54501.0
2                   Amridge University  1.0000    17.0      37621.0
3  University of Alabama in Huntsville  0.6429   669.0      61767.0
4             Alabama State University  0.3034  1921.0      34502.0
```

* Columns are accessible as indices, e.g. `colleges['median_earn']`, or as attributes of the data frame, e.g. `colleges.median_earn`
...
import pandas
pandas.read_csv("colleges.csv")

import pandas as pd
pd.read_csv("colleges.csv")

from pandas import *
read_csv("colleges.csv")
```
...
* Descriptive stats are available in `pandas` as data frame methods, e.g. `colleges.mean()`, `colleges.std()`
...
colleges.describe()
           ...
           rate       rnk  median_earn
count  2263.000  2263.000     2105.000
mean      0.521  1132.000    52699.362
std       0.214   653.412    16346.584
min       0.000    17.000    17360.000
25%       0.376   566.500    41855.000
50%       0.523  1132.000    50660.000
75%       0.670  1697.500    60509.000
max       1.000  2244.000   143372.000
```
...
<!-- SCAFFOLDING (Kyle to re-voice, delete this comment):
     Predict-then-run in chart form. Ask the room to sketch the shape of median earnings across
     2,105 colleges before showing the histogram: symmetric? piled up at one end? two humps?
     Actual shape: right-skewed (skew ~1.1) - most institutions between $40k and $60k, median
     $50,660, mean $52,699 pulled up by a long right tail out to $143k (MIT, Harvey Mudd, Olin,
     pharmacy schools). Mean > median is the tell. This is the same tail the box plot flags as
     outliers two slides later, and it sets up Assignment 3's "do your data appear skewed?" -->

- [KYLE: 2,105 colleges. Before I show you: sketch the shape you expect for median earnings of former students, 10 years after entry]
...
sns.histplot(data = colleges, x = "median_earn")
```
...
```python
sns.kdeplot(data = colleges, x = "median_earn", fill = True)
```
...
```python
sns.boxplot(data = colleges, y = "median_earn", color = "green")
```
...
```python
sns.violinplot(data = colleges, x = "median_earn", color = "cyan")
```
